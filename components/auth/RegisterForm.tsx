...
export default function RegisterForm() {
  ...
  return (
    <form onSubmit={submit} className="register-form flex flex-col gap-[18px]" noValidate>
      <div className="register-form-trial rounded-xl border border-emerald-100 bg-emerald-50 px-3.5 py-3 text-xs leading-5 text-emerald-800"><strong className="font-extrabold">Coba gratis</strong> · Buat hingga 3 dokumen dengan AI.</div>
      <button type="button" disabled={loading} onClick={googleRegister} className="register-form-oauth flex min-h-12 w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"><GoogleMark />Daftar dengan Google</button>
      <div className="register-form-divider flex items-center gap-3" aria-hidden="true"><span className="h-px flex-1 bg-slate-200" /><span className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">atau dengan email</span><span className="h-px flex-1 bg-slate-200" /></div>
      ...
      <div className="register-form-terms"><label className="flex cursor-pointer items-start gap-2.5 text-xs leading-5 text-slate-600"><input name="terms" type="checkbox" className="mt-0.5 h-4 w-4 shrink-0 rounded border-slate-300 accent-blue-600" /><span>Saya menyetujui Syarat Penggunaan dan Kebijakan Privasi Satuna Ajar.</span></label>{errors.terms && <p className="mt-1.5 text-xs font-medium text-red-600">{errors.terms}</p>}</div>
      ...
      <button disabled={loading} type="submit" className="register-form-submit min-h-12 w-full rounded-xl bg-blue-600 px-5 text-sm font-extrabold text-white shadow-sm transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">{loading ? 'Memproses...' : 'Buat akun gratis'}</button>
      <p className="register-form-login text-center text-sm text-slate-500">Sudah punya akun? <Link href="/login" className="font-extrabold text-blue-600 hover:text-blue-700">Masuk</Link></p>
    </form>
  );
}
